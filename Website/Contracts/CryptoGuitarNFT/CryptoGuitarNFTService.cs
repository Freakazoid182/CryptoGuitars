...
using CryptoGuitarNFT.ContractDefinition;

namespace CryptoGuitars.Contracts.CryptoGuitarNFT
{
    public partial class CryptoGuitarNFTService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, CryptoGuitarNFTDeployment cryptoGuitarNFTDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<CryptoGuitarNFTDeployment>().SendRequestAndWaitForReceiptAsync(cryptoGuitarNFTDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, CryptoGuitarNFTDeployment cryptoGuitarNFTDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<CryptoGuitarNFTDeployment>().SendRequestAsync(cryptoGuitarNFTDeployment);
        }

        public static async Task<CryptoGuitarNFTService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, CryptoGuitarNFTDeployment cryptoGuitarNFTDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, cryptoGuitarNFTDeployment, cancellationTokenSource);
            return new CryptoGuitarNFTService(web3, receipt.ContractAddress);
        }
        ...
        public CryptoGuitarNFTService(Nethereum.Web3.IWeb3 web3, string contractAddress)
        {
            Web3 = web3;
            ContractHandler = web3.Eth.GetContractHandler(contractAddress);
        }

        public Task<string> CANNOT_TRANSFER_TO_ZERO_ADDRESSQueryAsync(CANNOT_TRANSFER_TO_ZERO_ADDRESSFunction cANNOT_TRANSFER_TO_ZERO_ADDRESSFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<CANNOT_TRANSFER_TO_ZERO_ADDRESSFunction, string>(cANNOT_TRANSFER_TO_ZERO_ADDRESSFunction, blockParameter);
        }


        public Task<string> CANNOT_TRANSFER_TO_ZERO_ADDRESSQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<CANNOT_TRANSFER_TO_ZERO_ADDRESSFunction, string>(null, blockParameter);
        }

        public Task<string> NOT_CURRENT_OWNERQueryAsync(NOT_CURRENT_OWNERFunction nOT_CURRENT_OWNERFunction, BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<NOT_CURRENT_OWNERFunction, string>(nOT_CURRENT_OWNERFunction, blockParameter);
        }


        public Task<string> NOT_CURRENT_OWNERQueryAsync(BlockParameter blockParameter = null)
        {
            return ContractHandler.QueryAsync<NOT_CURRENT_OWNERFunction, string>(null, blockParameter);
        }
        ...
        public Task<string> ApproveRequestAsync(string approved, BigInteger tokenId)
        {
            var approveFunction = new ApproveFunction();
                approveFunction.Approved = approved;
                approveFunction.TokenId = tokenId;

             return ContractHandler.SendRequestAsync(approveFunction);
        }

        public Task<TransactionReceipt> ApproveRequestAndWaitForReceiptAsync(string approved, BigInteger tokenId, CancellationTokenSource cancellationToken = null)
        {
            var approveFunction = new ApproveFunction();
                approveFunction.Approved = approved;
                approveFunction.TokenId = tokenId;
        ...
        public Task<string> MintRequestAsync(string to, BigInteger tokenId, string uri)
        {
        ...
        public Task<TransactionReceipt> MintRequestAndWaitForReceiptAsync(string to, BigInteger tokenId, string uri, CancellationTokenSource cancellationToken = null)
        {
            var mintFunction = new MintFunction();
                mintFunction.To = to;
                mintFunction.TokenId = tokenId;
